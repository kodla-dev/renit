...
/**
 * Escapes a given value by wrapping it with a string that calls `$.escape`.
 *
 * @param {string} value - The value to be escaped.
 * @returns {string} - Returns the escaped value wrapped in `$.escape()`.
 */
...
/**
 * Generates a string representation of an object from an array of items.
 *
 * @param {Array<Object>} array - The array of items to process. Each item should have `name`,
 * `value`, and `disabled` properties.
 * @param {boolean} [close=true] - Whether to wrap the generated string in curly braces `{}`.
 * @returns {string} - A string representing the object, with items formatted as `name:value` pairs.
 */
export function generateStringObject(array, close = true) {
  let content = [];

  each(item => {
    if (!item.disabled) {
      if (item.name == item.value) {
        push(`${item.name}`, content);
      } else {
        push(`${item.name}:${item.value}`, content);
      }
    }
  }, array);

  content = join(RAW_COMMA, content);
